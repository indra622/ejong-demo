...
def process(i, threshold, logmel_data, outfile, model, wavpath):
    ...
    if unknown_flag == True:
        outfile.write(str(wavpath)+','+str(time(round(i*1.0,1))) + ',' + str(0.0) + ',' + 'unknown')
        outfile.write('\n')
        print(wavpath, time(round(i*1.0,1)), 'unknown')
    else:
        outfile.write(str(wavpath)+','+str(time(round(i*1.0,1))) + ',' + str(out_softmax[0][int(out_result)]) + ',' + out_classes[int(out_result)][0])
        outfile.write('\n')
        print(wavpath, time(round(i*1.0,1)), out_softmax[0][int(out_result)], out_classes[int(out_result)][0])
